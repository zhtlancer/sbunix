...
struct inode *path_lookup(struct inode *parent, const char *path)
{
	...
	if (path[0] == '\0' && parent != NULL)
		return get_inode(parent);
	...
	if (is_rootfs(parent))
		...
		inode = parent->fs_ops->path_lookup(parent, path);
...
{
	struct inode *inode = path_lookup(current->cwd, path);

	if (inode == NULL || inode->p_inode.type != IT_DIR) {
		put_inode(inode);
		return -1;
	}
